...
export function PostComponent({ post }: { post: PostType }) {
  ...
  const renderPostContent = () => {
    const isYouTubePost =
      post.post?.includes("youtube-video") ||
      post.post?.includes("youtube.com/embed") ||
      post.post?.includes("youtu.be");
  ...
  const handlePostingComment = async () => {
    ...
    commentOnPost(post.id, newCommentObj, post.posterUserId).then(
      async (saved) => {
  ...
  return (
    <>
      <Box className="mt-8">
        ...
        <OinkInfo
          visible={oinkInfoModalVisible}
          onClose={() => setOinkInfoModalVisible(false)}
        />
      </Box>
    </>
  );
