...
export default function SingleCommunity() {
  ...
  useEffect(() => {
    ...
    const normalizedCommunityId = Array.isArray(communityId)
      ? communityId[0]
      : communityId;
    console.log("Subscribing to posts for community:", normalizedCommunityId);

    const unsubscribe = subscribeToPostsByNewsfeedId(
      normalizedCommunityId,
      (updatedPosts: Post[]) => {
        console.log("Received updated posts:", updatedPosts.length);

        const normalizedPosts = updatedPosts.map((post) => ({
          ...post,
          featured: true,
        }));
